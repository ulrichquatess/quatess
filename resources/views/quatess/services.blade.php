...
<div class="cbp-item Branding"><a data-id="51" href="{{ url('services/'.$service->slug)}}" class="cbp-caption su-blog-img">
<div class="cbp-caption-defaultWrap"><img src="{{asset('images/services/'.$service->image)}}" alt="Digital Business Cards"></div><div class="cbp-caption-activeWrap">
...
</div></a><a href="{{ url('services/'.$service->slug)}}" class="cbp-l-grid-blog-title">{!! substr(strip_tags($service->title), 0, 90) !!} {!! strlen(strip_tags($service->title)) > 90 ? "...." : "" !!}</a>
<div class="su-pgrid-meta">
<span class="cbp-l-grid-blog-date">{{ date( 'M j Y h:ia ', strtotime( $service->updated_at ))}}</span>
<span class="cpb-category">Branding</span>
</div>
<div class="cbp-l-grid-blog-desc">{!! substr(strip_tags($service->content), 0, 90) !!} {!! strlen(strip_tags($service->content)) > 90 ? "...." : "" !!}</div>
</div>
@endforeach
